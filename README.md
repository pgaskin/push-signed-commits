...
- uses: pgaskin/push-signed-commits@v1.2.1
  with:
...
GITHUB_TOKEN=github_pat_xxx npx -y push-signed-commits@v1.2.1 -m 'commit message' username/repo master
```

```bash
# with a github app installation token (cli)
APP_PRIVATE_KEY="$(< private.pem)" npx -y push-signed-commits@v1.2.1 -C other-repo -m 'commit message' --app 1234 username/other-repo master
```

```bash
# as a library
npm install --save push-signed-commits@v1.2.1
```
...
- uses: pgaskin/push-signed-commits@v1.2.1
  with:
    ...
    # Authenticate as a GitHub App with the specified ID (App ID or Client ID).
    # The installation ID will be detected based on 'repository'. Overrides
    # 'github-token'. The app must have the 'contents:write' permission. If you
    # already have an app installation token, you can pass it via 'github-token'
    # instead.
    app-id: ''
...
```
usage: npx -y push-signed-commits@v1.2.1 [options] username/repository target_branch [revision]
  ...
  -A, --user-agent str          override the user agent for GitHub API requests (default "push-signed-commits/1.2.1")
...
```bash
GITHUB_TOKEN=github_pat_xxx npx -y push-signed-commits@v1.2.1 -m $'commit message subject\n\ncommit message body' username/repo master
```
...
```bash
GITHUB_TOKEN=github_pat_xxx npx -y push-signed-commits@v1.2.1 username/repo master HEAD@{u}..HEAD
```
...
```bash
# with a github app installation token (cli)
APP_PRIVATE_KEY="$(< private.pem)" npx -y push-signed-commits@v1.2.1 -C other-repo --app 1234 username/other-repo master HEAD
```
